...
# Constriction pattern: d1 -> d2 -> d1 over 40 μm, spaced 100 μm apart
CONSTRICTION_LENGTH = 40.0  # μm
CONSTRICTION_SPACING = 100.0  # μm


def get_diameter_at_position(
    position: float, length: float, d1: float, d2: float
) -> float:
    """Diameter at position along vessel. Periodic constriction pattern."""
    if length <= 0:
        return d1
    phase = position % CONSTRICTION_SPACING
    if phase < CONSTRICTION_LENGTH:
...
def calculate_integrated_resistance(
    length: float, d1: float, d2: float, num_points: int = 1000
) -> float:
    ...
    resistances = [
        resistance_integrand(pos, length, d1, d2) for pos in positions
    ]
...
def set_poiseuille_weights_with_constrictions(
    G: nx.MultiGraph, diameter_by_branch_order: dict
) -> dict:
...
def set_poiseuille_edge_weights(
    ...
    if edge_diameter <= 0:
        ...
        return results
    viscosity = calculate_viscosity(edge_diameter)
