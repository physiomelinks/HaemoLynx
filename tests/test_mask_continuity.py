...
from ImageLynx.graph import enforce_small_vessel_mask_continuity
from ImageLynx.graph import redefine_small_masks_from_large_tangential_contact
...
def test_continuity_respects_opposite_type_exclusion():
    ...
    # Opposite arteriole barrier in between should block bridge.
...
def test_continuity_endpoint_facing_gate_blocks_sideways_cylinders():
    ...
    # Source runs along x.
    small_ven = _cylinder_along_x(shape, z=12.0, y=10.0, radius=1.5, x0=3, x1=8)
    # Target runs along z (orthogonal); not facing source endpoint.
    zz, yy, xx = np.indices(shape, dtype=float)
    large_ven = (
    ...
    assert int(result["stats"]["venule"]["accepted_bridges"]) == 0
    assert int(result["stats"]["venule"]["rejected_reasons"].get("endpoint_facing_mismatch", 0)) >= 1
...
def test_sandwiched_component_reassignment_flips_middle_label():
    shape = (32, 24, 24)
    # Two venule cylinders with a gap.
    ...
    # Mislabelled middle segment currently in arteriole mask.
