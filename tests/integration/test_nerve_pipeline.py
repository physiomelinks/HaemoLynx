...
def test_nerve_pipeline_on_cropped_last_z_quarter_bottom_y_half():
    ...
    solved_graph = pipeline.image_to_model_pipeline(
        ...
        inlet_node_volumes=[((0.0, 0.0, 0.0), (float(cropped_z - 1), float(y_split - 1), float(cropped_x - 1)))],
        outlet_node_volumes=[((0.0, float(y_split), 0.0), (float(cropped_z - 1), float(cropped_y - 1), float(cropped_x - 1)))],
        inlet_nodes=[],
        outlet_nodes=[],
        inlet_p_bc=1000.0,
        outlet_p_bc=500.0,
