...
class PipelineConfig:
    ...
    vtk_output_prefix: Path = field(default_factory=lambda: Path("outputs/resistance_network"))
...
def 
    ...
    pipeline_config.vtk_output_prefix = Path(pipeline_config.vtk_output_prefix)
    ...
    if pipeline_config.do_skeletonize:
        ...
        skeleton = preprocessing.preprocess_skeleton_for_graph(
            skeleton,
            pipeline_config.min_branch_length=skel_config.min_branch_length,
            max_bridge_distance=skel_config.max_bridge_distance,
            component_connectivity=skel_config.component_connectivity,
            min_component_fraction=skel_config.min_component_percent / 100.0,
            skel_config.bundle_scan_size=skel_config.bundle_scan_size,
            skel_config.bundle_density_fraction=skel_config.bundle_density_fraction,
            skel_config.bundle_max_connections=skel_config.bundle_max_connections,
            skel_config.bundle_hub_min_spacing=skel_config.bundle_hub_min_spacing,
        )
        preprocessing.print_skeleton_connectivity_stats(
            "cleaned",
            skeleton,
    ...
    if pipeline_config.do_graph_building:
        ...
        if vis_config.visualize_overlay_preview and 'binary' in locals():
...
from dataclasses import dataclass, field
    ...
    graph_config.starting_nodes[:] = []
    graph_config.output_nodes[:] = []
    start_nodes, out_nodes = graph.select_boundary_terminal_nodes(
        G,
        image.shape,
        graph_config.edge_percent=graph_config.edge_percent,
        graph_config.end_percent=graph_config.end_percent,
        axis=graph_config.node_edge_axis,
    )
    graph_config.starting_nodes.extend(start_nodes)
    graph_config.output_nodes.extend(out_nodes)
    print(
        f"Auto-selected {len(graph_config.starting_nodes)} STARTING_NODES "
        f"(top {graph_config.edge_percent}%) and {len(graph_config.output_nodes)} OUTPUT_NODES "
        f"(bottom {graph_config.end_percent}%) along axis {graph_config.node_edge_axis}."
    )
    print(f"Starting nodes are: {graph_config.starting_nodes}")
    print(f"Output nodes are: {graph_config.output_nodes}")

    if graph_config.starting_nodes and graph_config.output_nodes:
        resistance_node_pair = (graph_config.starting_nodes[0], graph_config.output_nodes[0])
    ...
    if graph_config.starting_nodes:
        graph.assign_branch_orders(G, graph_config.starting_nodes)
...
if __name__ == "__main__":
    pipeline_config.plot_dir = BASE_PLOT_DIR / "carotid"
    ...
    carotid_image_to_model(
        image_path=target_input_mask_path, 
        pipeline_config.plot_dir=pipeline_config.plot_dir,
        skel_config.downsample_factor=SKELETON_DOWNSAMPLE_FACTOR,
        skel_config.use_padded_slicing=SKELETON_USE_PADDED_SLICING,
        skel_config.padded_slicing_padding=SKELETON_PADDED_SLICING_PADDING,
        skel_config.prune_mask_before=SKELETON_PRUNE_MASK_BEFORE_SKELETONIZATION,
        skel_config.sub_volume_percentage=SKELETON_SUB_VOLUME_PERCENTAGE,
        skel_config.sub_volume_offset_z=SKELETON_SUB_VOLUME_CENTER_OFFSET_Z,
        skel_config.sub_volume_offset_y=SKELETON_SUB_VOLUME_CENTER_OFFSET_Y,
        skel_config.sub_volume_offset_x=SKELETON_SUB_VOLUME_CENTER_OFFSET_X,
        vis_config.visualize_results=VISUALIZE_RESULTS,
        vis_config.visualize_mask_only=VISUALIZE_MASK_ONLY,
        vis_config.visualize_vedo=VISUALIZE_VEDO,
        vis_config.visualize_overlay_preview=VISUALIZE_OVERLAY_PREVIEW,
        vis_config.visualize_vedo_mode=VISUALIZE_VEDO_MODE,
        vis_config.visualize_vedo_smooth_iter=VISUALIZE_VEDO_SMOOTH_ITER,
        vis_config.visualize_vedo_spacing=VISUALIZE_VEDO_SPACING,
        vis_config.visualize_vedo_auto_spacing=VISUALIZE_VEDO_AUTO_SPACING,
        vis_config.visualize_vedo_opacity=VISUALIZE_VEDO_OPACITY,
        vis_config.visualize_mask_opacity=VISUALIZE_MASK_OPACITY,
        pre_config.median_filter_size=MEDIAN_FILTER_SIZE,
        pre_config.probability_smoothing_sigma=PROBABILITY_SMOOTHING_SIGMA,
        pre_config.morphological_opening_radius=MORPHOLOGICAL_OPENING_RADIUS,
        pre_config.enable_hysteresis_threshold=ENABLE_HYSTERESIS_THRESHOLD,
        pre_config.hysteresis_threshold_low=HYSTERESIS_THRESHOLD_LOW,
        pre_config.hysteresis_threshold_high=HYSTERESIS_THRESHOLD_HIGH,
        pre_config.enable_hole_filling=ENABLE_HOLE_FILLING,
        vis_config.visualize_post_processed_mask=VISUALIZE_POST_PROCESSED_MASK,
        pre_config.ilastik_vessel_channel=ILASTIK_VESSEL_CHANNEL,
        pre_config.enable_shannon_entropy=ENABLE_SHANNON_ENTROPY,
        pre_config.shannon_entropy_threshold=SHANNON_ENTROPY_THRESHOLD,
        graph_config.keep_largest_component_only=GRAPH_KEEP_LARGEST_COMPONENT_ONLY,
        skel_config.bundle_scan_size=SKELETON_BUNDLE_SCAN_SIZE,
        skel_config.bundle_density_fraction=SKELETON_BUNDLE_DENSITY_FRACTION,
        skel_config.bundle_max_connections=SKELETON_BUNDLE_MAX_CONNECTIONS,
        skel_config.bundle_hub_min_spacing=SKELETON_BUNDLE_HUB_MIN_SPACING
    )
